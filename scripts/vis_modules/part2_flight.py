...
def generate():
    ...
    for ax in axes:
        ax.axvspan(15.0, 19.0, color="#fef3c7", alpha=0.55, label="소화 약제 방출 구간 (15s~19s)" if ax == axes[0] else "")
        ax.grid(True, linestyle="--", alpha=0.5, color="#cbd5e1")
        ax.set_xlim(0, 34)
        
    axes[0].set_ylabel("위치 추종 오차 [m]\n(Position Error)", fontweight="bold")
    axes[0].set_title("소화 방식별 드론 6-DOF 비행 동역학 및 제어 시계열 비교 (D4 능동제어)", pad=12, fontweight="bold")
