...
작동 구간에서 일정 질량유량을 가정하면

\[
\dot m_c=-\frac{m_{emit}}{t_{act}}
\]

이다. 기체 총질량은 실제로 배출된 질량만큼만 줄어든다. 탑재량이 계획 배출량보다 많으면 남은 양은 귀환할 때까지 질량으로 유지된다.

펄스 분사의 충격량은

\[
J=\int_{t_0}^{t_1}\lVert F(t)\rVert dt
\]

로 계산한다. 접근 종료와 작동 종료 시각을 시간격자에 명시적으로 삽입하고, 펄스가 시간단계와 겹치는 실제 길이를 적분한다. 따라서 `dt_s`가 작동시간이나 펄스 주기를 나누어떨어뜨리지 않아도 질량과 충격량을 잃지 않는다.
